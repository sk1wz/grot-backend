...
Требует авторизации и возвращает цену, название и описание для каждого модуля.
...
### Изменить цену: администратор

```http
PATCH /checks/prices/{module}
Content-Type: application/json
```

Требует роль `ADMIN`. Цена передается целым числом и не может быть
отрицательной.
...
Пример: `PATCH /checks/prices/GIBDD`.
...
Возвращает суммарное число всех проверок в системе, включая проверки из batch,
и их распределение по модулям. Маршрут требует роль `ADMIN`.
